...
window.renderThemeTabs = (activeCategory) => {
    ...
    categories.forEach(cat => {
        ...
        html += `<button type="button" class="theme-category-btn ${activeClass}" onclick="window.renderThemeTabs('${cat}')">${cat}</button>`;
    });
    html += `</div>`;
        ...
        if (p.category === activeCategory) {
            html += `
                <button type="button" class="theme-preset-btn" 
                    ...
                    onclick="window.applyPreset('${key}')">
...
window.openThemeModal = () => {
    ...
    if (typeof window.openModal === 'function') window.openModal('themeModal');
};

window.closeThemeModal = () => { 
    if (typeof window.closeModal === 'function') window.closeModal('themeModal');
    window.initCustomTheme(); 
};
