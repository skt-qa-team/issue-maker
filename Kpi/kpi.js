...
window.QA_CORE.KPI = {
    ...
    switchTab: (tabId) => {
        window.QA_CORE.KPI.State.currentTab = tabId;
    ...
    clearTab: (tabId) => {
        ...
        window.QA_CORE.KPI.generate();
        if (window.QA_CORE.UI) window.QA_CORE.UI.showToast("초기화 되었습니다.", "success");
    },
    ...
    addTcRow: (data = {}) => {
        ...
        row.innerHTML = `
            <div class="tc-input-item">
                <select class="tc-poc" onchange="window.QA_CORE.KPI.generate()">
                    ${pocOptions}
                </select>
            </div>
            <div class="tc-input-item" style="flex:1.5;">
                <input type="text" class="tc-name" placeholder="티켓 이름" value="${data.name || ''}" oninput="window.QA_CORE.KPI.generate()">
            </div>
            <div class="tc-input-item" style="flex:1;">
                <input type="text" class="tc-ticket" placeholder="티켓 번호" value="${data.ticket || ''}" oninput="window.QA_CORE.KPI.generate()">
            </div>
            <div class="tc-input-item">
                <input type="number" class="tc-total" placeholder="건수" value="${data.total || ''}" min="0" oninput="window.QA_CORE.KPI.generate()">
            </div>
            <div class="tc-input-item tc-devices-label" style="min-width: 80px;">
                <label style="display:flex; align-items:center; gap:5px; font-weight:normal; width:auto; text-align:left;">
                    <input type="checkbox" class="tc-devices" onchange="window.QA_CORE.KPI.generate()" ${isChecked}> 단말 2대
                </label>
            </div>
            <div class="tc-actions">
                <button class="btn-tc-del" onclick="window.QA_CORE.KPI.removeTcRow(this)">🗑️</button>
            </div>
        `;
    ...
    generate: () => {
        try {
            ...
            window.QA_CORE.KPI.saveData();
        } catch (err) {
            if(window.QA_CORE.ErrorHandler) window.QA_CORE.ErrorHandler.handle(err, 'KPI Generate');
        }
    },
        ...
        try {
            ...
            if (totalDefect > 0) {
                ...
                if (blocker > 0 || critical > 0) {
                    ...
                    defectText += `꼼꼼한 리그레션 테스트를 매끄럽게 진행하여 서비스 전반의 완성도를 높이는 데 주력하였습니다.\n\n`;
                }
            } else {
            ...
            if(window.QA_CORE.ErrorHandler) window.QA_CORE.ErrorHandler.handle(err, 'KPI Generate Narrative');
        }
    },
        ...
        try {
            ...
            if (typeof firebase !== 'undefined' && firebase.auth) {
                ...
                if (user && !user.isAnonymous) {
                    firebase.database().ref(`users/${user.uid}/kpi`).set(data).catch(err => {
                        if(window.QA_CORE.ErrorHandler) window.QA_CORE.ErrorHandler.handle(err, 'KPI Save Firebase');
                    });
                }
            }
        } catch (err) {
            if(window.QA_CORE.ErrorHandler) window.QA_CORE.ErrorHandler.handle(err, 'KPI Save Data');
        }
    },
    ...
    loadLocal: () => {
        try {
            ...
            if (container) {
                container.innerHTML = ''; 
                if (saved.tcRows) saved.tcRows.forEach(row => window.QA_CORE.KPI.addTcRow(row));
            }
            window.QA_CORE.KPI.generate();
        } catch (err) {
            if(window.QA_CORE.ErrorHandler) window.QA_CORE.ErrorHandler.handle(err, 'KPI Load Local');
        }
    },
    ...
    copyReport: async () => {
        ...
        try {
            if (navigator.clipboard && window.isSecureContext) {
                ...
                if (window.QA_CORE.UI) window.QA_CORE.UI.showToast('📋 리포트가 복사되었습니다!', 'success');
            } else throw new Error();
        } catch (err) {
            window.QA_CORE.KPI.fallbackCopy(textToCopy);
        }
    },
    ...
    fallbackCopy: (text) => {
        ...
        try {
            ...
            if(window.QA_CORE.ErrorHandler) window.QA_CORE.ErrorHandler.handle(err, 'KPI Fallback Copy');
        }
