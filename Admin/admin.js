...
window.initAdminPanel = () => {
    const injectInterval = setInterval(() => {
        ...
        if (topBarBtns && !document.querySelector('.admin-btn-wrapper')) {
            ...
            wrapper.onclick = window.openAdminModal;
...
window.loadUsers = () => {
    ...
    firebase.database().ref('users').once('value').then(snapshot => {
        ...
        Object.keys(users).forEach(uid => {
            ...
            const displayName = (u.displayName && u.displayName !== 'undefined') ? window.escapeHtml(u.displayName) : (u.email ? window.escapeHtml(u.email.split('@')[0]) : '알 수 없음');
            const emailStr = (u.email && u.email !== 'undefined') ? window.escapeHtml(u.email) : '이메일 없음';
            ...
            div.innerHTML = `
                ...
                <div class="admin-action-group">
                    ...
                    <select onchange="window.changeUserStatus('${uid}', this.value)" class="admin-status-select">
